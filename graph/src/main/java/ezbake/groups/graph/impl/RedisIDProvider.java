...
public class RedisIDProvider implements GroupIDProvider {
    ...
    public static final String ID_DEF_VAL = "0";
    ...
    public void setCurrentID(long id) throws Exception {
        ...
        Jedis jedis = null;
        try {
            long currentId = idGetter.getCurrentId();
            if (id == currentId) {
                jedis = jedisPool.getResource();
                jedis.set(ID_REDIS_KEY, String.valueOf(id));
            } else {
                logger.error("Unable to set current id to {}. Conflicting value in database: {}", id, currentId);
            }
            setIdValid(true);
        } finally {
            if (jedis != null) {
